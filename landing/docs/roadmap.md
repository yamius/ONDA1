...
| 6.4 | **Inline glossary links on review pages.** Added contextual `[term](/glossary/slug)` links in the `content` markdown of 18 flagship reviews (HRV trackers, CGMs, vagus stim, cold plunge, EEG). **33 links** to 12 glossary terms. Only `content` renders Markdown; `summary`/`verdict` are plain text, left untouched. Product-prose reviews with no natural mechanism term (joovv, headspace, calm, sleepio) deliberately skipped to avoid forced linking. | `src/data/reviews/*.ts` (content field) | ≥ 30 new internal glossary backlinks. Verified 18 review pages now carry glossary links. | ✅ done 2026-05-29 |

### 6B — High-impact, needs a decision or larger effort

| # | Task | Impact | Effort | Blocker / decision needed |
|---|---|---|---|---|
| 6.5 | **Product images on review pages → DONE 2026-05-29 (Variant C, branded score cards).** Reviews/round-ups shipped with ZERO images. Decision: real product photos are a licensing project (affiliate feeds / press kits — deferred to a monetization track, see note below); AI generation is wrong for product-accurate reviews (hallucinated renders, trademark + trust risk). Interim chosen: programmatic branded score cards (1200×630) rendered SVG→PNG via sharp (already a dep — no AI, no new package). `scripts/generate-review-cards.ts` runs at build into `dist/images/reviews/<slug>.png` for every live review (name + brand + category + score badge) and round-up (title + N picks). Build-time gen → date-gated reviews get a card automatically the build after they go live; no committed binaries. Each page now has a real og/twitter image, Product.image, AND a visible hero `<img>`. Verified 88 cards, layout clears badge + wraps long names. | 🔴 High | done | ✅ done — cards (real photos = future affiliate track) |
| 6.6 | **Product schema `image` field → DONE.** meta-inject defaults `Product.image` (and og:image) to `/images/reviews/<slug>.png` when a review has no explicit `image:`. A future real photo set in the data file overrides it. | 🟡 Med | done | ✅ done 2026-05-29 |
| 6.7 | **Image sitemap for reviews + round-ups → DONE 2026-05-29.** Extended `scripts/sitemap.ts` with `buildImageBlockForReview` + `buildImageBlockForComparison` (mirroring the article hero block), wired into the URL map for EN `/reviews/<slug>` and `/reviews/compare/<slug>`. Each emits `<image:loc>` (branded card, or explicit photo when set) + `<image:title>` + `<image:caption>`. Image entries: 68 (articles) → 156 (+80 reviews, +8 round-ups). Verified title/caption render. | 🟡 Med | done | ✅ done 2026-05-29 |
| 6.8 | **Inline Author Person entity on review + round-up pages.** Review/CollectionPage schema referenced `author` by `@id`, but the `Person` node was only emitted on homepage + /about — a dangling @id on every review page. **Fixed:** inject the FULL canonical Person (not a sparse stub — these are YMYL health pages where author E-E-A-T is exactly what Google weighs; credentials + knowsAbout are the strong signal). Same @id → Google de-dupes to one node. Fires only on review/round-up paths. Verified: exactly 1 Person entity per page, homepage untouched. | 🟢 Low | 20 min | ✅ done 2026-05-29 |

### 6C — Performance / nice-to-have

| # | Task | Impact | Effort | Notes |
|---|---|---|---|---|
| 6.9 | **Bundle split — EVALUATED & DEFERRED 2026-05-29.** Measured the real picture before touching anything. Entry chunk `index-*.js` = 338KB raw / **107KB gzip — under the 150KB budget**, React included. An article page's STATIC HTML references only the entry + a 6KB preload; the page content is prerendered, so **LCP/FCP are already optimal and JS-independent**. The one real inefficiency: the article registry statically imports all 69 article bodies into one `articles-*.js` chunk (644KB raw / 162KB gzip) that loads lazily during hydration — so a single-article visitor eventually pulls all-article data. BUT this is POST-paint (content already visible), affecting only INP/TBT on slow mobile, and for a mostly-text article React-19 hydration is fast. Fixing it (per-slug dynamic glob imports across ArticlePage / GlossaryTermPage / ReviewPage / ComparisonPage / HeadToHeadPage, decoupling the client bundle from the registry that prerender/sitemap/llms/rag all depend on) is a High-risk refactor of load-bearing data architecture for a Low-impact, post-paint-only gain. Vendor-chunking React alone is marginal (already in the cached entry). Same disciplined call as 6.10 — defer unless INP shows up as a real Search Console / CrUX problem. | 🔵 Low | — | ⏸ deferred — first-paint already optimal, fix is high-risk/low-reward |
| 6.10 | **~~Render-blocking CSS → inline critical~~ — EVALUATED & REJECTED 2026-05-29.** Built end-to-end (Beasties post-prerender pass over all 856 HTML files; ran clean, 855 inlined). Then measured and reverted — net-NEGATIVE for this site. Why: app ships ONE shared 86KB / 13KB-gzip stylesheet, and pages are fully prerendered (whole article/review body in the HTML). Beasties can't detect the fold on static HTML, so it inlines critical CSS for the entire page ≈ 44KB raw / ~16KB gzip ≈ most of the sheet. Because the sheet is shared, `pruneSource` can't strip those rules (would break other pages), so the full sheet STILL loads async → CSS ships TWICE. Measured homepage: HTML gzip 2.65KB → 18.9KB; total CSS-related bytes/load ~15.5KB → ~31.8KB (+16KB gzip). Only gain is one saved RTT on first paint — not worth doubling CSS bytes on a sheet already 13KB gzip. A real win needs per-page CSS bundles (major architecture change) — unjustified for a Low-impact item. | 🔵 Low | — | ❌ rejected — net-negative, measured |
...
| 6.12 | **~~NewsArticle for round-ups~~ — WON'T DO.** Reviewed 2026-05-29 and rejected. Round-ups already emit `CollectionPage + ItemList + FAQPage + BreadcrumbList` — which is exactly Google's documented canonical shape for a ranked "best X" list. Adding `NewsArticle`/`Article` would (a) misuse `NewsArticle`, which requires Google News Publisher Center approval + journalistic standards that product round-ups don't meet — a misuse signal, not a boost; (b) risk Article+CollectionPage type conflict where Google ignores both; (c) add nothing real — freshness is driven by `dateModified` (already in CollectionPage) + sitemap `<lastmod>` (already correct) + actual content updates, none of which an Article type improves. Original audit note was over-eager. | — | — | ❌ won't do — current schema already optimal |
| 6.13 | **FAQ placement audit — DONE 2026-05-29, no change needed.** Audited all FAQ surfaces (articles, round-ups, head-to-heads). All correct: FAQ Q&A text is in the prerendered DOM (articles/H2H use `<details>`/`<summary>` — answers render server-side regardless of open state, so crawler reads them; round-ups use open `<div>`+`<h3>`+`<p>`, fully visible). Visible text matches the FAQPage JSON-LD (same data source). Bottom-of-page position is normal and fine — Google does NOT require FAQ above the fold; it only discounts FAQ that is hidden-not-in-DOM or mismatched with schema, neither of which applies. Note for expectations: Google deprecated FAQ rich snippets in 2023 for non-gov/health sites, so the FAQPage schema rarely yields visible SERP rich results for ONDA anymore — but it still feeds AI answer engines (Perplexity / ChatGPT / Google AI Overviews parse FAQPage Q&A) and is valid + harmless, so keep it. | 🔵 Low | Audit | ✅ audited — no change needed |

### Phase 6 status (closed 2026-05-29)

Practical SEO audit complete. **Shipped:** 6.1 (verified already-correct),
6.2 title-truncation fix, 6.3 TITLE_MAX 65, 6.4 review glossary links (33),
6.5 branded score cards (88), 6.6 Product.image, 6.7 image-sitemap entries
(68→156), 6.8 inline Author Person, 6.11 SpeakableSpec on reviews, 6.13 FAQ
(audited, no change). **Rejected after measurement:** 6.10 critical-CSS
(net-negative), 6.12 NewsArticle (schema already optimal). **Deferred:** 6.9
bundle split (first paint already optimal; fix is high-risk/low-reward).
**Open monetization track:** real product photos via affiliate feeds to
eventually replace the score cards (needs owner accounts).
