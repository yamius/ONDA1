...
| 6.7 | **Image sitemap for reviews + round-ups.** `sitemap.xml` has 68 `<image:loc>` entries — all articles. Now that review/round-up cards exist (6.5), extend the sitemap generator to add `<image:loc>` for each `/reviews/*` + `/reviews/compare/*` page → faster Google Images discovery. | 🟡 Med | 30 min | ☐ next — now unblocked |
