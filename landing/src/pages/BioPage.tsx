...
export function BioPage() {
  ...
  const [dlPlatform, setDlPlatform] = useState<string>('')
  const [dlSubmitted, setDlSubmitted] = useState(false)
  const [dlError, setDlError] = useState<string | null>(null)
  const [dlLoading, setDlLoading] = useState(false)

  const handleDlSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setDlError(null)
    setDlLoading(true)
  ...
  const handleDlClose = () => { setDlOpen(false); setDlPlatform(''); setDlError(null) }
  ...
  return (
    <div className="-mt-6 min-h-screen text-white" style={{ background: 'linear-gradient(160deg,#1a0a2e 0%,#0d0620 50%,#12082a 100%)' }}>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="bio-waitlist-title"
      >
        <div
          ...
          <div className="h-[270px] flex flex-col shrink-0">
            {!dlSubmitted ? (
              <form onSubmit={handleDlSubmit} className="flex flex-col flex-1">
                ...
                />
                {dlError && <p className="mb-4 text-sm text-rose-400" role="alert">{dlError}</p>}
                <button
                  type="submit"
                  disabled={dlLoading}
