...
// during prerender (Node), so a bare `import.meta.env.X` would throw there. The
// HDR URLs are only consumed inside the lazy WelcomeScene (never mounted at
// prerender), so an empty base here is harmless; the browser build has the env.
