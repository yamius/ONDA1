...
export function calculatePracticeOnd(metrics: PracticeMetrics): OndReward {
  ...
  // Check if we have valid stress/energy values (either real or simulated from UI)
  const hasValidMetrics = stressBefore !== null && stressAfter !== null && 
                          energyBefore !== null && energyAfter !== null &&
                          stressBefore > 0 && energyBefore > 0;

  // If no valid metrics at all, use fallback simulation
  if (!hasValidMetrics) {
  ...
  // Use provided metrics (real from sensors OR simulated from UI)
  ...
  return {
    ...
    completionPercent: Math.round(completionRatio * 100),
    performancePercent: Math.round(((stressScore + energyScore) / 2) * 100),
    isSimulated: !hasRealMetrics // True if using UI-simulated values, false if from real sensors
  };
}
