 ...
 *
 * Method (chosen for MAX accuracy after a literature/adversarial review, not a
 * 1:1 port of the web /bio tool):
 *   raw {t,v} red-channel means  → resample to a uniform grid (never trust a
 *   fixed fps) → linear detrend → zero-phase Butterworth band-pass 0.6–3.5 Hz
 *   → PRIMARY estimator = frequency-domain spectral peak (Goertzel sweep +
 *   parabolic interpolation + octave/harmonic guard) → CONFIRM with
 *   autocorrelation (parabolic lag) → commit a value only when the two agree
 *   and the signal-quality index passes; otherwise return null (hold/blank).
 *
 * Everything here is framework-free pure math (reuses src/hooks/dsp.ts), so it
 * is unit-testable and identical on web and in the iOS WKWebView.
 *
 * References behind the choices: Allen 2007 (PPG review); Coppetti 2017
 * (smartphone HR accuracy varies hugely with the quality layer); contact-with-
 * torch ⇒ RED channel (Verkruysse 2008 "green is best" is the non-contact case);
 * Elgendi skewness SQI; ANSI/AAMI EC-13 (≤5 bpm) as the bar.
 */

import { goertzelPower, clamp01, ewma } from '../hooks/dsp';
 ...
 * stays visible. A true band-pass (not the web tool's moving-average-only
 * detrend) removes the HF sensor/quantisation/flash-flicker noise that
 * fabricates false peaks.
 */
...
export function spectralHr(
  ...
  // Peak selection with HPS-style harmonic reinforcement — the octave (2×)
  // guard. A TRUE fundamental f is reinforced by energy at its 2nd harmonic 2f;
  // a 2nd harmonic masquerading as the spectral max has no in-band super-
  // harmonic (4f) to reinforce it, so it loses. This robustly avoids the
  // classic "sharp/dicrotic pulse reads as 2× HR" failure, including when the
  // band-pass tilts power toward the harmonic. The raw-power term is retained
  // so a near-pure sinusoid still selects its own peak.
  let peakPowGlobal = 0;
  ...
  // 3-bin peak band ÷ total = "concentration" (reuses the pattern from useVitals)
  let band = peakPow;
...
export function autocorrHr(
  ...
  // which grows with HR: ~2 bpm @60, ~8 bpm @120)
  let lag = bestLag;
...
// NOTE on breathing: camera users get breathing the SAME way the watch does —
// useVitals runs its Goertzel RSA analysis on the HR series fed into
// heartRateStore. So ppgCore stays pulse-only; no separate envelope estimator
...
/**
 * EMA whose responsiveness RISES with confidence and FALLS on weak signal,
 * with a per-update slew cap. Keep the effective time-constant well under the
 * ~10 s RSA cycle so the exhale dip in pulse stays visible.
 */
