...
| Part 9 | Ментальная симуляция и нейронная пластичность | ✅ Готов |
| Part 10 | Нейроэндокринология влияния и голосовой резонанс | ✅ Готов |
| Part 11 | Социальная когниция и эмпатическая регуляция | ✅ Готов |
| Part 12 | Нейронная синхронизация и коллективный интеллект | ✅ Готов |
...
### Цветовая схема части и блоки базовых практик

Правила применения цвета части (`activeCircuit`) к карточкам базовых практик и кнопкам. Код: `src/onda-level1-demo_27.tsx`, секция списка практик (~строки 5145–5285).

#### 1. Блок карточки практики (контейнер)

- **Завершённая практика** (`isCompleted`): для **всех** частей одинаково — зелёный акцент: `border-emerald-500/50 bg-emerald-500/10`. Так пользователь видит прогресс (например, блок с зелёной обводкой и индикатором %).
- **Незавершённая практика**: цвет **зависит от части** — бордер и hover в цвет части (например, Part 2 — `border-cyan-500/30`, Part 4 — `border-teal-500/30`).  
  **Исключение:** Part 3 в коде использует для незавершённых карточек серый: `border-gray-500/30 hover:border-gray-400/50` (не amber). При желании выровнять под палитру части — заменить на amber.

#### 2. Кнопка действия «Улучшить» / «Начать практику»

- Цвет **всегда берётся из палитры части**: Part 2 — cyan, Part 3 — amber, Part 4 — teal и т.д.
- Для Part 3: завершённая — `bg-amber-600/50 border border-amber-400/50`, не завершённая — `bg-amber-600 hover:bg-amber-700`.
- При добавлении новой части нужно добавить ветку `activeCircuit === N` с соответствующими классами кнопки (~строки 5265–5277).

#### 3. Ссылка «Session History» (история сессий)

- Текст ссылки «▶ Session History (N)» может быть в цвет части (Part 2 — cyan) или нейтральным (Part 3 — gray). При унификации палитры части — задавать цвет части (~строки 5201–5208).

#### 4. Внутренние карточки сессий (в раскрытой истории)

- Бордер карточек сессий в цвет части: Part 2 — `border-cyan-500/20`, Part 3 — `border-amber-600/20` (~строки 5218–5224). Для новой части — добавить условие с выбранным цветом.

#### Сводка по элементам (пример Part 3)

| Элемент | Завершённая практика | Незавершённая практика |
|--------|----------------------|-------------------------|
| Блок карточки | Зелёный (emerald) | Серый в коде (при желании — amber) |
| Кнопка «Improve» / «Start» | Amber (полупрозрачная) | Amber (сплошная) |
| Ссылка Session History | — | Серая (при желании — amber) |
| Карточки сессий в истории | Бордер amber | — |

При добавлении новой части (Part 5, 6, …) для базовых практик нужно добавить ветки `activeCircuit === N` в: контейнер карточки, кнопку действия, ссылку Session History и бордер карточек сессий (см. «Где нужно задать цвета для новой части» и поиск по `activeCircuit === 3` в этом блоке).
...
9a. **Блоки базовых практик** (~строки 5149–5224, 5265–5277): контейнер карточки (бордер незавершённой), ссылка «Session History», бордер карточек сессий в истории, кнопка «Улучшить»/«Начать практику» — везде добавить ветку `activeCircuit === 5` с цветом части (см. раздел «Цветовая схема части и блоки базовых практик» выше).
...
7. ✅ (Опционально) Добавить EXR-панораму для фона экрана практики — см. раздел «EXR-панорамы для практик» ниже
...
## 🌌 EXR-панорамы для практик

### Что это

Во время практики (экраны `intro` и `active`) вместо стандартного фона с пульсирующими кругами отображается 360° HDR-панорама через Three.js. Компонент — `src/components/WelcomeScene.tsx`.

### Где хранятся файлы

**Supabase Storage**, бакет `hdr`, папка `hdr_p1`:
```
hdr/hdr_p1/exr_p1_01.exr   → практика p1-1
hdr/hdr_p1/exr_p1_02.exr   → практика p1-2
...
```

Базовый URL формируется из переменной окружения `VITE_SUPABASE_URL`:
```typescript
const HDR_BASE = `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/hdr/hdr_p1`;
```

### Где задаётся маппинг

Файл `src/components/WelcomeScene.tsx` — словарь `PRACTICE_EXR`:

```typescript
export const PRACTICE_EXR: Record<string, string> = {
  'p1-1': `${HDR_BASE}/exr_p1_01.exr`,
  'p1-2': `${HDR_BASE}/exr_p1_02.exr`,
  // Новые практики добавлять здесь ↓
};
```

Если для практики нет записи в `PRACTICE_EXR` — отображается стандартный фон (пульсирующие круги). Отсутствие записи не вызывает ошибок.

### Как добавить панораму для новой практики

1. **Сжать** файл `.exr` кодеком DWAA, качество ~45 → размер 4K-панорамы ≈ 2.5 МБ
   (необработанный файл может весить 25+ МБ — так нельзя!)
...
- Загрузчик: `THREE.EXRLoader`
- **Обязательно:** `loader.setDataType(THREE.FloatType)` — иначе NaN/Inf проверки не работают (с `HalfFloat` `isNaN()` всегда возвращает `false`)
- После загрузки: очистка пикселей (NaN → 0, Inf → 0, отрицательные → 0)
- Маппинг: `EquirectangularReflectionMapping`
- Генерация освещения: `PMREMGenerator` (не `hdr.mapping`, а ручной `fromEquirectangular`)
- Tone mapping: `ACESFilmicToneMapping` с exposure 1.0

### Текущее состояние панорам

| Практика | Файл | Статус |
|----------|------|--------|
| p1-1 | `exr_p1_01.exr` | ✅ Загружен |
| p1-2 | `exr_p1_02.exr` | ✅ Загружен |
| Остальные | — | Стандартный фон |
