...
**ВАЖНО:** Уровни и Части связаны автоматически в коде!

### Логика переключения (реализовано в `onda-level1-demo_27.tsx`):

1. **При выборе Chapter → переключение на первую Part этого уровня:**
   ```typescript
   const firstLevelOfChapter = (chapter - 1) * 3 + 1;
   // Chapter 1 → Part 1, Chapter 2 → Part 4, Chapter 3 → Part 7, Chapter 4 → Part 10
   ```

2. **При выборе Part → переключение на соответствующий Chapter:**
   ```typescript
   const chapterForLevel = Math.ceil(level / 3);
   // Parts 1-3 → Chapter 1, Parts 4-6 → Chapter 2, Parts 7-9 → Chapter 3, Parts 10-12 → Chapter 4
   ```
