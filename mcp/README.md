...
| `ga4_breakdown` | One event split by a dimension (share by unique users); standard GA4 fields (`pagePath`, `landingPage`, …) pass as-is; `stream` (website/app/all), `exclude_country`, optional `conversion_event` (per-row converting users + rate), `limit` | GA4 Data API |
