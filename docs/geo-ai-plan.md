...
- 2026-09-06: **Cornerstone bridge-entity pages DONE** (commit c7b6c135). `/hrv-biofeedback` (definition → how it works → evidence → HRV does/doesn't → tracking vs biofeedback → how ONDA implements → devices → limitations → Research[cited DOI/PMID] → FAQ) and `/resonance-breathing` (science of ~0.1Hz resonance, find-your-frequency, evidence, practice, ONDA). Both Article + FAQPage JSON-LD, reuse evidence.ts refs, honest, wired everywhere. These are the site's #1 scientific+product landing pages — answer "what is HRV biofeedback", "what does ONDA do" and "is ONDA evidence-based" in one place.
