...
| **1.8.2** | 2026-06-18 | ✅ **LIVE** (per owner) | Ветка `claude/practice-funnel-analytics`, влита в main. Инструментирование воронки: `practice_complete` +`time_percent/metrics_source/is_first`; пейвол расцеплен с мёртвым `first_practice_complete` → армится на первом `practice_complete` (`onda_paywall_armed`); `first_practice_quality` на paywall_view+purchase; 5 GA4 custom dimensions. **Оффер снова показывается.** Reviewer Notes = камера 1.8.1 + абзац про подписку |
| **1.8.3** | 2026-06-20 | TestFlight | `results_view` event + честный захват pulse-trend (без изменений UI); честный сигнал A/B/C на экране результата, «Try again» скрыт в онбординге; результатные тексты доведены до всех 5 локалей (+es, +zh) |
| **1.8.4** | 2026-06-21 | TestFlight | Два фикса достижимости состояния B: захват HR привязан к моменту фиксации камеры, а не к t=0; источник HR в сессии сделан «липким» — снятие пальца перед «End» больше не переключает результат на A/C |
| **1.8.5** | 2026-06-24 | TestFlight | Событие `home_view` закрывает слепую зону воронки после пейвола (`first_run` дизъюнктен с `purchase`); явный выход «Продовжити безкоштовно» с `paywall_dismiss{action}`; переделанный экран результата (тексты в 5 локалях, акценты, дышащая волна) и единый orb-дизайн first-run → intro → results |
| **1.8.6** | 2026-06-25 | TestFlight | Онбординг открывается практикой «Відчуття буття» вместо Micro-Breath; экран результата разделён по `cameFromFirstRun` — онбординг сохраняет новый честный дизайн, практики из хаба получают обратно свой геймифицированный экран (редизайн протекал во все практики). Build 2 |
| **1.8.7** | 2026-06-25 | TestFlight | **Изменений кода нет.** Чистый бамп номера после возврата `CURRENT_PROJECT_VERSION` к 1 (номер сборки ставит fastlane из таймстампа). Содержимое идентично 1.8.6 build 2 |
| **1.8.8** | 2026-09-01 | build pending | Метка внутреннего трафика как Firebase **user property** `internal` (7 тапов по строке версии в Меню); строка версии в Меню добавлена и сама по себе. Нужна, чтобы собственные прогоны не двигали воронку при ~50 установках/мес |
...
### 1.8.8 — build pending (2026-09-01)

**Что в билде:** метка внутреннего трафика. `AnalyticsService` выставляет Firebase
**user property** `internal` (`true`/`false`) на каждом запуске, читая флаг из
`localStorage`; переключается семью тапами по строке версии в Меню, состояние
подтверждается бейджем `INTERNAL`. Строка версии (`CFBundleShortVersionString`
через `App.getInfo()`) добавлена в Меню и полезна сама по себе — при разборе
жалоб версия нужна всегда.

**Почему user property, а не параметр события:** `first_open` — первый шаг
воронки активации — авто-событие Firebase, оно не проходит через `track()`.
Параметр в общем payload до него не долетел бы никогда, и первый шаг остался бы
нефильтруемым. То же с `session_start` и `screen_view`. User property SDK цепляет
ко всем событиям, включая авто-собираемые.

**Честная граница:** метка действует только с этой сборки. Всё, что уже записано
в GA4 — включая собственные прогоны по 1.8.5–1.8.7 из TestFlight — разделить
задним числом нельзя. При ~50 установках/мес несколько своих проходов заметно
двигают проценты, поэтому сборка и понадобилась срочно.

**Что НЕ вошло намеренно:** `app_version` в общий payload не выносился. Поле
`app_version` в приложении хранит `VITE_BUILD_NUMBER` (= `github.run_number`), а не
маркетинговую версию, и в Firebase/GA4 не попадает вообще; GA4 получает настоящую
версию через встроенное измерение `appVersion`. Свой параметр продублировал бы его
и сжёг слот кастомного измерения.

**Проверка:** DebugView на TestFlight-сборке **не работает** (нет
`-FIRDebugEnabled`) — пустой экран там не доказывает отсутствие события. Либо
запуск из Xcode с этим флагом, либо обычные отчёты GA4 через 24–48 ч по измерению
`Internal traffic` (scope User, зарегистрировано 2026-08-31).

**Коммиты:** `a5a9ac9d` (метка), `8253d998` (бамп версии).

---

### 1.8.7 — TestFlight (2026-06-25) — без изменений кода

**Что в билде:** ничего нового. Между бампом 1.8.6 и бампом 1.8.7 нет ни одного
фичевого коммита: 1.8.7 — чистый бамп номера сразу после `6cb80e57`, вернувшего
`CURRENT_PROJECT_VERSION` к 1 (номер сборки ставит fastlane из таймстампа, а не
проект). Содержимое идентично 1.8.6 build 2.

**Коммиты:** `6cb80e57`, `a2f09414`.

---

### 1.8.6 — TestFlight (2026-06-25), build 2

**Что в билде:**
- Онбординг открывается практикой p1-2 «Відчуття буття / Sense of Being» вместо
  Micro-Breath (переставлен приоритет free-id в `featuredPracticeId`).
- Экран результата разделён по `cameFromFirstRun`: онбординг сохраняет новый
  orb-дизайн, а базовые практики, запущенные из хаба, получают обратно свой
  **исходный геймифицированный** экран (✨, начисленные OND, Quality/Time, звёзды,
  новый рекорд, `finalPhrase`) — восстановлен из `565d3db7`. Причина: редизайн
  протекал во все базовые практики, хотя предназначался только онбордингу.
- `CURRENT_PROJECT_VERSION` 1 → 2, чтобы прикрепить новый билд к ревью 1.8.6.

**Коммиты:** `4fc383c8` (бамп), `82489db2`.

---

### 1.8.5 — TestFlight (2026-06-24)

Самая содержательная версия диапазона: закрыта слепая зона воронки, убран
тёмный паттерн на пейволе и переделан первый экран результата.

**Аналитика — `home_view` (`c3a1c0cb`).** После `paywall_view` воронка прыгала
сразу в `purchase`, и было не видно, сколько людей доходят до приложения **не
купив**. Добавлено событие входа в хаб:
- `source: first_run` — пейвол онбординга закрыт **без** покупки. Покупка
  переключает `isPremium` и закрывает модалку сама, поэтому здесь событие не
  шлётся: `home_view{first_run}` и `purchase` остаются **дизъюнктными**, и
  «молчаливый выход» считается как `paywall_view − purchase − home_view{first_run}`.
- `source: relaunch` — возвращающийся пользователь попадает в хаб при запуске.
- Шлётся один раз на вход, а не на каждый перерендер. `menu` намеренно не
  инструментирован — воронке нужен только `first_run`.

**Пейвол — явный выход (`c264d69c`).** Единственным выходом без покупки был
крестик в углу — скрытая дверь, бьющая по доверию. Добавлена вторичная кнопка
«Продовжити безкоштовно» под CTA пробного периода: тонкий контур, спокойный цвет,
по ширине контента (намеренно **не** во всю ширину и не залитая), тап-таргет
≥44pt. Действие то же, что у крестика. `paywall_dismiss` теперь несёт
`action: close | continue_free`, так что видно, каким выходом пользуются. Копия во
всех 5 локалях; формулировка честная — три базовые практики действительно
бесплатны, без заявлений о «полном доступе даром».

**Экран результата (`4591098c`, `46a93941`, `66e450e8`, `ec0d8123`).** Итог
нескольких итераций, включая одну проверенную на живом TestFlight:
- ✨-эмодзи заменён на бренд-визуал когерентности (орб + две синхронные волны,
  чистый CSS/SVG), блок `finalPhrase` убран, честный сигнал A/B/C поднят под
  заголовок.
- Язык переписан в сторону «тренировка, а не трекинг»; акценты на числах и
  ключевых терминах через маркер `[[...]]` (без новых зависимостей и без разметки
  внутри строк, работает во всех локалях).
- Волна «дышит» на ~5.5 цикла/мин (11-секундный резонанс); анимация перенесена на
  обёртку `<div>` с принудительным GPU-слоем — иначе замерзала в WKWebView.
- **Откат внутри версии:** разделение forward-path на отдельную акцентную строку
  (`r_path_punch`) в реальном рендере читалось плохо и дублировало фразу про
  coherence/resting-HRV, а под русским блоком проваливалось в английский. Свёрнуто
  обратно в одно предложение во всех 5 локалях.
- Честность формулировок: coherence — это **живой** сигнал (с часами), а
  многодневный тренд — resting-HRV (реальный SDNN), а не «тренд coherence по дням»,
  которого у нас нет.

**Единый orb-дизайн (`64e9fc15`).** Общий компонент `<CoherenceOrb>` заменил три
встроенные копии; орб стоит в пиксельно одинаковой позиции на всех трёх экранах
first-run → intro → results, поэтому «не прыгает» между переходами. Всё новое
закрыто условием `cameFromFirstRun` — практики из хаба не затронуты. В онбординге
скрыты крестик и пилюли длительности/награды; при 50% качества защёлкивается
зелёная галочка и на 3 с показывается тост «Практика пройдена» (ключ
`practices.passed`, 5 локалей).

**Коммиты:** `c3a1c0cb` (несёт и бамп 1.8.4 → 1.8.5), `c264d69c`, `4591098c`,
`46a93941`, `66e450e8`, `ec0d8123`, `64e9fc15`, `2f10377e` (uk: «завершена» →
«закінчена»).

---

### 1.8.4 — TestFlight (2026-06-21)

**Что в билде:** два фикса, делающие результат честным.
- Состояние B стало достижимым: захват HR привязан к моменту фиксации камеры, а не
  к `t=0` (`e99f048e`).
- Источник HR в сессии сделан «липким»: снятие пальца перед нажатием «End» больше
  не переключает результат на A/C (`7dca3bd9`).

**Коммиты:** `e145b194` (бамп), `e99f048e`, `7dca3bd9`.

---

### 1.8.3 — TestFlight (2026-06-20)

**Что в билде:**
- Событие `results_view` + честный захват pulse-trend, без изменений UI
  (`f4c29cb1`).
- Честный сигнал A/B/C на экране результата; «Try again» скрыт в онбординге
  (`565d3db7`).
- Результатные тексты доведены до всех 5 локалей — добавлены es и zh
  (`5a4d5d6e`).

**Коммиты:** `faec0399` (бамп), `f4c29cb1`, `565d3db7`, `5a4d5d6e`.

---

### 1.8.2 — submitted 2026-06-18, released ✅
...
**Outcome:** ✅ **released / LIVE** (per owner). Funnel instrumentation now in prod — the post-first-practice paywall actually shows (armed on practice_complete), and practice_complete / paywall_view / purchase carry the new params. Watch GA4 for the quality→purchase curve + skip/cta split. first_practice_complete{70} kept as a reference metric.
...
**Решение по копирайту (как фактически в ASC):** 1.8.0 уже live (камера у людей) → What's New = короткая reliability-нота. **Reviewer Notes оставлены ПОЛНЫЕ (камера-пульс из 1.8.0)** — владелец решил не сокращать: проверенный аппрувом текст страхует на случай свежего ревьюера, и тема камеры им уже одобрена (не «переоткрытие»).
...
- **Keywords:** meditation,biofeedback,dopamine,detox,resonance,anxiety,stress,biohacking,polyvagal,sleep,calm
- **Support URL:** https://onda-life.com/contact · **Marketing URL:** https://onda-life.com
- **Copyright:** © 2026 ONDA Life
- **App Review contact:** ONDA Life · info@onda-life.com · +34 678 011 500
- **Sign-in required:** нет (галка снята)
- **Release:** авто-релиз после аппрува, дата-гейт 2026-06-13 13:00 (local); средняя оценка — сохранить

<details>
<summary>Reviewer Notes (как отправлено — полные, камера-пульс)</summary>
...
### 1.7.9 — submitted 2026-06-04, approved 2026-06-05 ✅
...
**Outcome:** ✅ **APPROVED 2026-06-05** (submitted 2026-06-04) — clean approval. Practice-audio resume + watch dead-man's-switch; `audio` background mode removed (no 2.5.4), no new permissions/data, demo-login не требовался.
